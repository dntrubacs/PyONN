...
class PhysicalDiffractiveLayer:
    ...
    def plot_phase_map(self) -> None:
        ...
        plt.title(
            f"Phase Map (measured in radians) placed at"
        ...
        # plot the phase map
        plt.imshow(X=self.phase_map.detach().cpu().numpy(), extent=limits)
        plt.xticks(x_ticks)
        plt.yticks(y_ticks)
        plt.colorbar()
        plt.show()
    ...
    def find_output(
        ...
        if plot:
            ...
            # title and axis labels
            plt.title(f"Intensity map placed at" f" z={z_coordinate}")
            plt.ylabel("y coordinate (m)")
            plt.xlabel("x coordinate (m)")
